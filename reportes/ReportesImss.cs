    ...
    public class ReportesImss
    {
        ...
        public static string GenerarReporteIncapacidades(int idUsuario, DateTime fecI, DateTime fecF, int idEmpresa = 0, string pathFolder = "", string pathDescarga = "")
        {
            ...
            using (var context = new RHEntities())
            {
                ...
                itemEmpresa = listaEmpresa.FirstOrDefault(x => x.IdEmpresa == idEmpresa);
                ...
                listaIncapacidades = (from i in context.Incapacidades
                                      where arrayIdEmpresa.Contains(i.IdEmpresaFiscal)
                                            && i.FechaInicio >= fechaI && i.FechaInicio <= fechaF && sucursales.Contains(i.IdSucursal)
                                      select i).ToList();
                ...
                var arrayIdEmpleado = listaContratos.Select(x => x.IdEmpleado).ToArray();
        ...
        public static string GenerarReporteIncapacidades2(int idUsuario, DateTime fecI, DateTime fecF, int idEmpresa = 0, string pathFolder = "", string pathDescarga = "")
        {
            ...
            using (var context = new RHEntities())
            {
                ...
                var arrayPeriodos = listaTimbrados.Select(x => x.IdPeriodo).Distinct().ToArray();//devuelve un array con los periodos de la lista de timbrados
                var arrayNominas = listaTimbrados.Select(x => x.IdNomina).ToArray();//devuelve los id de las nominas de la lista de timbrados


                var arrayContratos = (from n in context.NOM_Nomina
                                      where arrayNominas.Contains(n.IdNomina)
                                      select n.IdContrato).ToArray(); //devuelve un array con los id de los contratos de las nominas si estas conciden con el id de arrayNominas

                listaContratos = (from c in context.Empleado_Contrato
                                  where arrayContratos.Contains(c.IdContrato)
                                  select c).ToList();//devuelve una lista de empledo_contrato de acuerdo a el array con los contratos

                var arrayEmpleados = listaTimbrados.Select(x => x.IdEmpleado).Distinct().ToArray();//array con los id de empleados en base a la lista de timbrados

                listaEmpleados = (from e in context.Empleado
                                  where arrayEmpleados.Contains(e.IdEmpleado)
                                  select e).ToList();//lista de empleados base al array de empledos 

                listaPeriodos = (from p in context.NOM_PeriodosPago
                                     where arrayPeriodos.Contains(p.IdPeriodoPago)
                                     select p).ToList();//lista de periodos en base al array de id de periodos
